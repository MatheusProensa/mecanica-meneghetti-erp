...
/** Envia uma foto/comprovante vinculado a uma dívida (Devedores) pro bucket privado. */
export async function uploadDividaFoto(
  fileName: string,
  bytes: Buffer,
  contentType: string
): Promise<string> {
  const supabase = getClient();
  const { error } = await supabase.storage.from(DIVIDA_FOTOS_BUCKET).upload(fileName, bytes, {
    contentType,
    upsert: false,
  });
  if (error) throw error;
...
export async function deleteDividaFoto(path: string): Promise<void> {
  const supabase = getClient();
  await supabase.storage.from(DIVIDA_FOTOS_BUCKET).remove([path]);
}

/** Versão em lote, para a galeria de fotos da dívida. */
export async function getSignedDividaFotoUrls(paths: string[]): Promise<Record<string, string>> {
  if (paths.length === 0) return {};
  const supabase = getClient();
  const { data, error } = await supabase.storage
