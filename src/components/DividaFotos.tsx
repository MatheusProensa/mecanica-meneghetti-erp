...
import { addAnexoDivida, deleteAnexoDivida } from "@/app/(protected)/devedores/actions";

export default function DividaFotos({
  dividaId,
  fotos,
  readOnly = false,
}: {
  ...
  const addAnexoWithId = addAnexoDivida.bind(null, dividaId);
    ...
    <div>
      ...
      {fotos.length > 0 && (
        <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-4">
          {fotos.map((foto) => {
            const deleteWithId = deleteAnexoDivida.bind(null, foto.id, dividaId);
            return (
              <div key={foto.id} className="group relative aspect-square overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
                {foto.url ? (
                  ...
                  <img src={foto.url} alt="Foto da dívida" className="h-full w-full object-cover" />
                ) : (
